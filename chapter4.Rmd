...
## Bonus: LDA with k-means clusters as target classes

For bonus points, let's perform k-means on the original standardized Boston dataset, and then perform LDA using the clusters as target classes.

```{r}
# run k-mean with 3 clusters
km3 <- kmeans(boston_scaled, centers = 3)

# add the clusters as a variable to the data
boston_scaled$cluster <- km3$cluster

# run lda with clusters as the target
lda.clust <- lda(cluster ~., data = boston_scaled)

lda.clust

# define the colors for the plot
cols <- c("green", "blue", "purple")
cols1 <- cols[boston_scaled$cluster]

# plot the lda results
plot(lda.clust, dimen= 2, col = cols1, pch = cols1)
lda.arrows(lda.clust, myscale = 3)

```

The LDA seems to separate the clusters pretty well. From the plot we can see that the proportion of black residents is perhaps the most influential together with crime rate and proportion of non-retail business acres. Cluster 1 is best described as having a low proportion of black residents, low crime rate and low proportion of non-retail business acres. Cluster 2 also has low proportion of black residents and lower crime rate, but high proportion of non-retail business acres. Cluster 3 has higher proportion of black residents, high crime rate and high proportion of non-retail business acres.
